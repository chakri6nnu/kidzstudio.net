...
class ExamCrudController extends Controller
{
    ...
    public function settings($id)
    {
        ...
        return Inertia::render('Admin/Exam/Settings', [
            ...
            'editFlag' => true,
            'settings' => [
                'auto_duration' => $exam->settings['auto_duration'] ?? true,
                'auto_grading' => $exam->settings['auto_grading'] ?? true,
                'cutoff' => $exam->settings['cutoff'] ?? 60,
                'enable_section_cutoff' => $exam->settings['enable_section_cutoff'] ?? false,
                'enable_negative_marking' => $exam->settings['enable_negative_marking'] ?? false,
                'restrict_attempts' => $exam->settings['restrict_attempts'] ?? false,
                'no_of_attempts' => $exam->settings['no_of_attempts'] ?? null,
                'disable_section_navigation' => $exam->settings['disable_section_navigation'] ?? false,
                'disable_question_navigation' => $exam->settings['disable_question_navigation'] ?? false,
                'disable_finish_button' => $exam->settings['disable_finish_button'] ?? false,
                'hide_solutions' => $exam->settings['hide_solutions'] ?? false,
                'list_questions' => $exam->settings['list_questions'] ?? true,
                'shuffle_questions' => $exam->settings['shuffle_questions'] ?? false,
                'show_leaderboard' => $exam->settings['show_leaderboard'] ?? true,
            ],
        ]);
    }
